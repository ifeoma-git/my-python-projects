...
A Program that keeps track of the contents of a moving box.
It does that by taking note of items added or removed from the moving box.
"""
...
class MovingBox:
    ...
    def add_item(self, item_name: str, amount: int):
        if item_name in self._items:
            self._items[item_name] += amount
        else:
            self._items[item_name] = amount

    def remove_item(self, item_name: str, amount: int):
        if item_name not in self._items or self._items[item_name] < amount:
            return False
        self._items[item_name] -= amount
    ...
    def item_count(self, item_name: str):
    ...
    def transfer_item(self, target_box, item_name: str, amount: int):
        if self.remove_item(item_name, amount):
            target_box.add_item(item_name, amount)
            return True
        return False

    def search_item(self, item_name: str):
        return self.item_count(item_name)
